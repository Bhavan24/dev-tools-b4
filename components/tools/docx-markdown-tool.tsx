...
interface DocxMarkdownToolProps {
  toolId: string
}

type Tab = 'docx-to-md' | 'md-to-docx'

export function DocxMarkdownTool({ toolId }: DocxMarkdownToolProps) {
  const [tab, setTab] = useState<Tab>('docx-to-md')

  return (
    <div className="space-y-6">
      <div className="flex gap-1 p-1 bg-secondary rounded-xl w-fit">
        <button
          onClick={() => setTab('docx-to-md')}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'docx-to-md' ? 'bg-background shadow text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
        >
          DOCX → Markdown
        </button>
        <button
          onClick={() => setTab('md-to-docx')}
          className={`px-5 py-2 rounded-lg text-sm font-medium transition-all ${tab === 'md-to-docx' ? 'bg-background shadow text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
        >
          Markdown → DOCX
        </button>
      </div>

      {tab === 'docx-to-md' ? <DocxToMarkdownPanel /> : <MarkdownToDocxPanel />}
    </div>
  )
}
...
function DocxToMarkdownPanel() {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      ...
      const response = await fetch('/api/tools/docx-to-markdown', {
        method: 'POST',
        body: form,
      })
      if (!response.ok) {
  ...
  const downloadMarkdown = () => {
    ...
    a.href = url
    a.download = fileName.replace(/\.docx$/i, '.md') || 'output.md'
    a.click()
      ...
      {docxUrl && (
        <div className="p-4 bg-secondary rounded-xl flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-medium text-foreground text-sm">DOCX ready</p>
              <p className="text-xs text-muted-foreground">Click to download your Word document</p>
            </div>
          </div>
